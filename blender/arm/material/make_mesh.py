...
def make_forward_mobile(con_mesh):
    ...
    if '_Sun' in wrd.world_defs:
        ...
        if is_shadows:
            ...
            frag.add_uniform(f'sampler2D {shadowmap_sun_tr}')
            ...
            if '_CSM' in wrd.world_defs:
                ...
                if '_Legacy' in wrd.world_defs:
                    frag.write(f'    svisibility = float(texture({shadowmap_sun}, {shadowmap_sun_tr}, vec2(lPos.xy)).r > lPos.z - shadowsBias, opacity != 1.0);')
                else:
                    frag.write(f'    svisibility = texture({shadowmap_sun}, {shadowmap_sun_tr}, vec3(lPos.xy, lPos.z - shadowsBias), opacity != 1.0).r;')
                frag.write('}')
...
def make_forward_base(con_mesh, parse_opacity=False, transluc_pass=False):
    ...
    if is_shadows_atlas:
        shadowmap_sun = 'shadowMapAtlasSun' if not is_single_atlas else 'shadowMapAtlas'
        shadowmap_sun_transparent = 'shadowMapAtlasSunTransparent' if not is_single_atlas else 'shadowMapAtlasTransparent'
        frag.add_uniform('vec2 smSizeUniform', '_shadowMapSize', included=True)
    ...
    if '_VoxelGI' in wrd.world_defs:
        if parse_opacity:
            ...
            frag.write("    indirect += textureLod(voxels_specular, texCoord, 0.0).rgb * specular * voxelgiRefl * (1.0 - roughness);")
    ...
    if parse_opacity and '_VoxelRefract' in wrd.world_defs:
        ...
        frag.write('    indirect = mix(traceRefraction(wposition, n, voxels, voxelsSDF, normalize(eye - wposition), ior, roughness, clipmaps, gl_FragCoord.xy).rgb, indirect, opacity);')
        frag.write('    direct = mix(traceRefraction(wposition, n, voxels, voxelsSDF, normalize(eye - wposition), ior, roughness, clipmaps, gl_FragCoord.xy).rgb, direct, opacity);')
        frag.write('}')
