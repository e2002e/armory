...
class Inc {
	...
	public static function computeVoxelsTemporal() {
		...
		kha.compute.Compute.setTexture(voxel_ta1, rts.get(voxtex).image, kha.compute.Access.Read);
		kha.compute.Compute.setTexture(voxel_tb1, rts.get(voxtexLast).image, kha.compute.Access.Read);
	...
	#if (rp_voxels == "Voxel GI")
	public static function computeVoxelsLight(voxelsLightOut:String) {
		var rts = path.renderTargets;
	 	var res = Inc.getVoxelRes();
