...
float sampleVoxel(sampler3D voxels, const vec3 pos, vec3 dir, const uvec3 indices, const float lod) {
 	...
 	vec3 tc = pos;
 	tc.x += indices.x;
 	opac += dir.x * textureLod(voxels, tc, lod).r;
 	tc = pos;
 	tc.x += indices.y;
	opac += dir.y * textureLod(voxels, tc, lod).r;
	tc = pos;
	tc.x += indices.z;
	opac += dir.z * textureLod(voxels, tc, lod).r;
	return opac;
}
#endif
#endif
...
vec4 traceCone(sampler3D voxels, vec3 origin, vec3 n, vec3 dir, const float aperture, const float maxDist, const vec3 clipmap_center) {
    ...
    while (sampleCol.a < 1.0 && dist < maxDist && clipmap_index0 < voxelgiClipmapCount) {
		...
		mipSample = sampleVoxel(voxels, samplePos, dir, indices, lod);
		...
		if(clipmap_blend > 0.0) {
			...
			vec4 mixSampleNext = textureLod(voxels, samplePosNext, 0.0);
